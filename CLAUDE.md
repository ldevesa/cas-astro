...
- [x] **Envío doble del formulario — vendedores + marketing (03/08):** se agregó el equivalente al "Mail 1 / Mail 2" que tenían en Contact Form 7 (WordPress) — cada envío del formulario ahora manda **2 emails independientes**: uno a `CONTACT_TO` (vendedores, sin datos de UTM/origen) y otro a `CONTACT_TO_MARKETING` (nueva variable, con esos datos, para medir). Cada uno tiene su propio Mailjet→Resend por separado, así que uno puede fallar sin afectar al otro — el formulario muestra éxito si al menos uno de los dos llegó. Si `CONTACT_TO_MARKETING` no está configurada, simplemente no se manda ese segundo email, sin romper el flujo. `CONTACT_BCC` ahora aplica solo al email de marketing (antes aplicaba al único email que existía).
  ...
  - **Feature pendiente, no implementada (a pedido explícito de no hacerlo por ahora):** selector en el formulario para elegir a qué destinatario/vendedor le llega el mensaje (existía como idea en WordPress, sin llegar a activarse ahí tampoco). Retomar si en algún momento lo piden — no confundir con el campo "Servicio de interés" que ya existe hoy, que es solo informativo y no enruta el email a nadie en particular.
- [x] **Códigos de seguimiento (GTM, Search Console, scripts sueltos) editables desde Sanity (03/08):** nuevo documento singleton `configuracionSeguimiento` (`studio/schemaTypes/configuracionSeguimiento.ts`, mismo patrón que `paginaHome`), con 3 campos: `googleTagManagerId`, `googleSiteVerification` (código de verificación de Search Console) y `scriptsPersonalizados` (HTML/JS crudo, para pegar cualquier snippet suelto que no pase por GTM). `Layout.astro` los inyecta condicionalmente: el script de GTM y la meta tag de verificación van en `<head>`, el `<noscript>` de GTM justo después de `<body>`, y `scriptsPersonalizados` se renderiza sin escapar (`set:html`) al final del `<head>` — es contenido de confianza cargado desde el Studio por el equipo, no input de visitantes, a diferencia del `escapeHtml()` que sí se usa en el formulario de contacto. **Se recomendó GTM como "paraguas"** (un solo ID acá, Analytics/Pixel/Hotjar se agregan después desde el panel de GTM sin volver a tocar el sitio) en vez de un campo por servicio, porque el usuario no opera GTM directamente — solo recibe snippets sueltos de terceros, así que el campo de scripts personalizados cubre esos casos sin necesitar entender cada herramienta.
  - **Pendiente manual:** el filtro del webhook de Sanity→Cloudflare hay que actualizarlo a mano en sanity.io/manage (no se puede vía código) para incluir `configuracionSeguimiento` — si no, publicar esta configuración no dispara rebuild. Ver el gotcha de arriba, mismo patrón de siempre.
  - **Segundo campo agregado — scripts en 2 partes:** el usuario preguntó qué hacer con snippets que vienen en 2 mitades (una para `<head>`, otra para `<body>`, como el propio GTM). Se agregó `scriptsPersonalizadosBody`, inyectado justo después de que abre `<body>` (mismo lugar que el `<noscript>` de GTM) — el campo original (`scriptsPersonalizados`) quedó renombrado en el Studio a "— head" para dejar claro cuál es cuál.
